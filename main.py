...
if __name__ == '__main__':
    ...
    crawler = Crawler(url=url, parser=ICKMITLParser)
    crawler.run()
    for item in crawler.get_crawled_data():
       log.debug(f'Item: {len(item.professors)} {str(item)}')

    url = 'http://catalog.mit.edu/subjects/'
    crawler = Crawler(url=url, parser=MITParser, max_depth=2)
    crawler.run()
    for item in crawler.get_crawled_data():
        log.debug(f'Item: {len(item.professors)} {str(item)}')

    url = 'http://guide.berkeley.edu/courses/'
    crawler = Crawler(url=url, parser=UCBerkeleyParser, max_depth=2)
    crawler.run()
    for item in crawler.get_crawled_data():
        log.debug(f'Item: {len(item.professors)} {str(item)}')
